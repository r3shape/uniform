...
u8 _sprite_system_impl(u32 entity) {
    r3_core->graphics.push_pipeline(&(R3_Render_Call){
        ...
        .model = (ecsx->has_component(R3_TRANSFORM2D, entity)) ? &transform_store.model[entity] : &identity,
...
u8 _transform_system_impl(u32 entity) {
    transform_store.velocity[entity] = mathx->vec.scale2(
        transform_store.velocity[entity],
        transform_store.speed[entity]
    );

    transform_store.location[entity] = mathx->vec.add2(
    ...
    transform_store.model[entity] = mathx->mat.mult4(
        transform_store.model[entity],
        mathx->mat.rotz4(transform_store.rotation[entity])
    );
...
u8 _r3_register_component2D(void) {
    ...
    if (!ecsx->register_component(R3_SPRITE2D, &sprite_store, _add_sprite_impl, _rem_sprite_impl, (COMPONENT_GET_FPTR)_get_sprite_impl)) {
        printf("failed to register R3_Sprite2D component!\n");
        return LIBX_FALSE;
    }
    
    if (!ecsx->register_component(R3_TRANSFORM2D, &transform_store, _add_transform_impl, _rem_transform_impl, (COMPONENT_GET_FPTR)_get_transform_impl)) {
        printf("failed to register R3_Transform2D component!\n");
        return LIBX_FALSE;
    }
    
    if (!ecsx->register_system(R3_SPRITE2D, "render2D", _sprite_system_impl)) {
        printf("failed to register R3_Sprite2D component systems!\n");
        return LIBX_FALSE;
    }
    
    if (!ecsx->register_system(R3_TRANSFORM2D, "transform2D", _transform_system_impl)) {

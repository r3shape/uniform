...
u8 _register_event_impl(u16 event_code) {
    ...
    internal_event_state.callbacks[event_code] = structx->array.create_array(sizeof(R3_Event_Callback), R3_CALLBACK_MAX);
...
u8 _unregister_event_impl(u16 event_code) {
    ...
    if (internal_event_state.callbacks[event_code]) {
        structx->array.destroy_array(internal_event_state.callbacks[event_code]);
...
u8 _push_event_impl(u16 event_code, R3_Event data) {
    ...
    Array_Head head = structx->array.get_array_head(internal_event_state.callbacks[event_code]);
...
u8 _register_callback_impl(u16 event_code, R3_Event_Callback callback) {
    ...
    structx->array.push_array(internal_event_state.callbacks[event_code], &callback);
    return LIBX_TRUE;
}
...
u8 _unregister_callback_impl(u16 event_code, R3_Event_Callback callback) {
    ...
    LIBX_FORI(0, head.count, 1) {
        if (internal_event_state.callbacks[event_code][i] == callback) {
            ...
            structx->array.pull_array(internal_event_state.callbacks[event_code], i, &callback);
            return LIBX_TRUE;
        }
    }
    return LIBX_FALSE;

...
R3_Shader _create_shader_impl(cstr vertex, cstr fragment) {
    ...
    R3_Shader shader = {.uniforms = libx->dsx.array.create_hash_array(32)};
    if (!shader.uniforms) {
    ...
    if (!compile) {
        ...
        libx->dsx.array.destroy_hash_array(shader.uniforms);
        return (R3_Shader){0};
    }
...
u8 _set_uniform_impl(R3_Shader* shader, R3_Uniform* uniform) {
    ...
    return libx->dsx.array.put_hash_array(shader->uniforms, uniform->name, uniform);
}

u8 _send_uniform_impl(R3_Shader* shader, str name) {
    if (!shader || !name) return LIBX_FALSE;    // error: null ptr!
    
    R3_Uniform* uniform = libx->dsx.array.get_hash_array(shader->uniforms, name);
...
void _push_pipeline_impl(R3_Render_Call* call) {
    ...
    libx->dsx.array.push_array(_graphics_api->pipeline.calls, call);
}
...
void _flush_pipeline_impl(void) {
    ...
    Array_Head head = libx->dsx.array.get_array_head(_graphics_api->pipeline.calls);
    R3_Shader* shader = _graphics_api->pipeline.shader;
    LIBX_FORI(0, head.count, 1) {
        R3_Render_Call call;
        libx->dsx.array.pull_array(_graphics_api->pipeline.calls, 0, &call);
        ...
        R3_Uniform** uniforms = (R3_Uniform**)libx->dsx.array.get_hash_array_values(shader->uniforms);
        LIBX_FORI(0, shader->uniforms->meta.count, 1) {
            _graphics_api->send_uniform(shader, (str)uniforms[i]->name);
        }
        libx->dsx.array.destroy_array(uniforms);
...
u8 _init_pipeline_impl(R3_Render_Mode mode, R3_Shader* shader, Mat4 proj) {
    ...
    _graphics_api->pipeline.calls = libx->dsx.array.create_array(sizeof(R3_Render_Call), 1024);
...
u8 _init_camera_impl(Vec3 eye, Vec3 center, Vec3 up) {
    ...
    _graphics_api->camera.direction = libx->mathx.vec.norm3(libx->mathx.vec.sub3(eye, center));
    _graphics_api->camera.right = libx->mathx.vec.norm3(libx->mathx.vec.cross3(up, _graphics_api->camera.direction));
    _graphics_api->camera.up = libx->mathx.vec.cross3(_graphics_api->camera.direction, _graphics_api->camera.right);
    
    // update the view matrix with camera vectors
    _graphics_api->camera.view = libx->mathx.mat.lookat(
        _graphics_api->camera.eye,
        libx->mathx.vec.add3(_graphics_api->camera.eye, _graphics_api->camera.direction),
...
void _translate_camera_impl(i8 x, i8 y, i8 z) {
    if (x > 0) {
        _graphics_api->camera.eye = libx->mathx.vec.add3(_graphics_api->camera.eye, libx->mathx.vec.scale3(libx->mathx.vec.norm3(libx->mathx.vec.cross3(
            _graphics_api->camera.direction,
            _graphics_api->camera.up
        )), _graphics_api->camera.speed));
    }
    else if (x < 0) {
        _graphics_api->camera.eye = libx->mathx.vec.sub3(_graphics_api->camera.eye, libx->mathx.vec.scale3(libx->mathx.vec.norm3(libx->mathx.vec.cross3(
            _graphics_api->camera.direction,
            _graphics_api->camera.up
        )), _graphics_api->camera.speed));
    }
    ...
    if (y > 0) {
        _graphics_api->camera.eye = libx->mathx.vec.add3(
            _graphics_api->camera.eye,
            libx->mathx.vec.scale3(_graphics_api->camera.up, _graphics_api->camera.speed)
        );
    }
    else if (y < 0) {
        _graphics_api->camera.eye = libx->mathx.vec.sub3(
            _graphics_api->camera.eye,
            libx->mathx.vec.scale3(_graphics_api->camera.up, _graphics_api->camera.speed)
        );
    }
    ...
    if (z > 0) {
        _graphics_api->camera.eye = libx->mathx.vec.add3(
            _graphics_api->camera.eye,
            libx->mathx.vec.scale3(_graphics_api->camera.direction, _graphics_api->camera.speed)
        );
    }
    else if (z < 0) {
        _graphics_api->camera.eye = libx->mathx.vec.sub3(
...
void _update_camera_impl(void) {
    _graphics_api->camera.direction = libx->mathx.vec.norm3(libx->mathx.vec.vec3(
        cosf(libx->mathx.scalar.radians(_graphics_api->camera.yaw)) * cosf(libx->mathx.scalar.radians(_graphics_api->camera.pitch)),
        sinf(libx->mathx.scalar.radians(_graphics_api->camera.pitch)),
        sinf(libx->mathx.scalar.radians(_graphics_api->camera.yaw)) * cosf(libx->mathx.scalar.radians(_graphics_api->camera.pitch))
    ));
    _graphics_api->camera.right = libx->mathx.vec.norm3(libx->mathx.vec.cross3(_graphics_api->camera.direction, libx->mathx.vec.vec3(0, 1.0, 0)));
    _graphics_api->camera.up = libx->mathx.vec.norm3(libx->mathx.vec.cross3(_graphics_api->camera.right, _graphics_api->camera.direction));
...
u8 _r3_cleanup_graphics(_r3_graphics_api* api) {
    ...
    if (api->pipeline.calls) libx->dsx.array.destroy_array(api->pipeline.calls);
